...
export const createOrder = async (req: Request, res: Response) => {
    try {
        const { user_id, deliveryMethod, paymentMethod, address } = req.body.user_id;
        ...
        let sum = 0;
        let quantity = 0;
        const total_price = cart_items.forEach(item => sum += item.price);
        const total_amount = cart_items.forEach(item => quantity += item.quantity);
